"""
Master Incremental Processing Pipeline for Adding 2,000 Products.
Processes ONLY the new products:
Phase A: Qwen2-VL-2B AI Image Captioning
...
NEW_CSV_PATH = ROOT / "data" / "processed" / "new_products_2000.csv"
CAPTIONS_CHECKPOINT = ROOT / "data" / "processed" / "new_captions_checkpoint.json"
NORMALIZED_CHECKPOINT = ROOT / "data" / "processed" / "new_normalized_checkpoint.json"
...
def run_phase_a():
    print("\n" + "=" * 80)
    print("PHASE A: AI Image Captioning with Qwen2-VL-2B (Incremental)")
    print("=" * 80)
    ...
    if len(pending_df) == 0:
        print("All 2,000 images already captioned! Skipping Phase A.")
        return captions
    ...
    for idx, row in pending_df.iterrows():
        ...
        try:
            ...
            captions[pid] = f"{row['product_name']} {row['main_category']}"
...
def run_phase_b(captions):
    print("\n" + "=" * 80)
    print("PHASE B: Text Keyword Normalization with Qwen2.5-0.5B (Incremental)")
    print("=" * 80)
    ...
    if len(pending_pids) == 0:
        print("All 2,000 items already normalized! Skipping Phase B.")
        return normalized
...
def run_phase_c(captions, normalized):
    print("\n" + "=" * 80)
    print("PHASE C: Appending to SQLite Database & Master Checkpoints")
    print("=" * 80)
...
def run_phase_d(normalized):
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    # Get all products in exact order
    ...
    for i in range(0, len(pending_rows), batch_size):
        ...
        time.sleep(1.0)
        if (i // batch_size) % 10 == 0 or (i + batch_size) >= len(pending_rows):
...
def run_phase_e():
    ...
    for i in range(0, len(pending_rows), batch_size):
        ...
        time.sleep(1.0)
        if (i // batch_size) % 5 == 0 or (i + batch_size) >= len(pending_rows):
...
def main():
    print("*" * 80)
    print("STARTING INCREMENTAL CATALOG EXPANSION PIPELINE (+2,000 Products)")
    print("*" * 80)
    ...
    # Step 4: SQLite Database Update
    run_phase_c(captions, normalized)

    # Step 5: BGE-Large FAISS Update
    run_phase_d(normalized)

    # Step 6: CLIP Image Embeddings Update
    run_phase_e()

    total_time = time.time() - start_time
    print("\n" + "=" * 80)
    print(f"CATALOG EXPANSION PIPELINE COMPLETED SUCCESSFULLY IN {total_time / 60:.2f} MINUTES!")
    print("=" * 80)
